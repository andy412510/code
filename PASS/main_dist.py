...
def get_args_parser():
    ...
    parser.add_argument('--width', default=128, type=int, help="""Width of image""")
    ...
    parser.add_argument('--output_dir', default='/home/andy/PASS-reID-main/logs/', type=str, help='Path to save logs and checkpoints.')
    parser.add_argument('--data_path', type=str, metavar='PATH',
                        default='/home/andy/ICASSP_data/data/data_folder/market1501/train_folder')
    # /home/andy/ICASSP_data/data/data_folder/market1501/train_folder
    # /home/andy/ICASSP_data/data/LUP
...
def get_train_loader(args, dataset, height, width, batch_size, workers, sampler, trainset=None):

    train_transformer = DataAugmentationDINO(
        (height,width),
...
def train_dino(args):
    ...
    # ============ LUP dataset ============
    dataset = torch_datasets.ImageFolder(args.data_path, transform=transform)
    if args.filter_path != '':
    ...
    #     os.path.join(args.output_dir, "vits.pth"),
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader,
    ...
    for it, (images, _) in enumerate(metric_logger.log_every(data_loader, 200, header)):
        ...
        # file_names = data_loader.dataset.imgs[it]
        # print(file_names)
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            loss = dino_loss(teacher_output_g, student_output_g, student_output_pt1, student_output_pt2,
                             student_output_pt3, epoch)
...
class DataAugmentationDINO(object):
    ...
    def __call__(self, image):
        ...
        for _ in range(self.local_crops_number // 3):
            crops.append(
                self.local_transfo(transforms.functional.crop(image, int(0.25 * height), 0, int(0.5 * height), width)))
        for _ in range(self.local_crops_number // 3):
            crops.append(
                self.local_transfo(transforms.functional.crop(image, int(0.5 * height), 0, int(0.5 * height), width)))
        return crops
