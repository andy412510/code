...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn = (q @ k.transpose(-2, -1)) * self.scale # matrix multiplication between the last two dimensions of the tensors
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=(224,224), patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm, pretrained_path='', hw_ratio=0, pretrain=True, **kwargs):
        super().__init__()
    ...
    def forward(self, x, part_index):
        ...
        for blk in self.blocks:  # blk= one of self.blocks Block
            x = blk(x)
        x = self.norm(x)
        if part_index==None:
